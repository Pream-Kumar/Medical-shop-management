...
if(!isset($_SESSION['user_session'])){  //User_session
  ...
  backup_tables('localhost','root','','simplepharmacy');
...
function backup_tables($host,$user,$pass,$name,$tables = '*')
{
	...
	//save file
	if(!file_exists("C:\SPMS_DB_Backup")){

		mkdir("C:\SPMS_DB_Backup");
	}
	    $date = date("dMY");
	    @$invoice_number = $_GET['invoice_number'];


        if(!file_exists('C:\SPMS_DB_Backup\db-backup-'.$date.'-'.implode(',',$tables).'.sql')){
		$handle = fopen('C:\SPMS_DB_Backup\db-backup-'.$date.'-'.implode(',',$tables).'.sql','w+');
